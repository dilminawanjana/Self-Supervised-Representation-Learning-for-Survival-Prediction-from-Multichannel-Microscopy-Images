...
class SimCLRViewTransform:
    def __init__(
        ...
        transform = [
            T.RandomCrop(size=input_size, 
                        #  scale=(min_scale, 1.0),
                         ),
            random_rotation_transform(rr_prob=rr_prob, rr_degrees=rr_degrees),
            T.RandomHorizontalFlip(p=hf_prob),
            T.RandomVerticalFlip(p=vf_prob),
            # T.RandomApply([color_jitter], p=cj_prob), # Those are for 3 channel images
            # T.RandomGrayscale(p=random_gray_scale), # Those are for 3 channel images
