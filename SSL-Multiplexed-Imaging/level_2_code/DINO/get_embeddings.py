...
def main(args):
    ...
    model = DINO.load_from_checkpoint(args.checkpoint_path)
    model.eval()
    ...
    np.save(args.save_dir / f'embeddings_{args.checkpoint_path.stem}.npy', embeddings)
    np.save(args.save_dir / f'names_{args.checkpoint_path.stem}.npy', filenames)
...
def parse_arguments():
    ...
    parser.add_argument('--input_size', type=int, default=14, help='Input size (default: 256)')
